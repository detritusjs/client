...
export type InteractionCommandClientCommandCheck = (
	context: InteractionContext,
	command: InteractionCommand
) => Promise<boolean> | boolean
export type InteractionCommandClientCommandCancel = (
	context: InteractionContext,
	command: InteractionCommand
) => Promise<any> | any
export type InteractionCommandClientInteractionCheck = (
	context: InteractionContext
) => Promise<boolean> | boolean
export type InteractionCommandClientInteractionCancel = (
	context: InteractionContext
) => Promise<any> | any

export interface InteractionCommandClientAddOptions
	extends InteractionCommandOptions {
	_class?: any
}

export interface InteractionCommandClientRunOptions
	extends ClusterClientRunOptions {
...
export class InteractionCommandClient extends EventSpewer {
	...
	constructor(
		...
		this.checkCommands =
			options.checkCommands || options.checkCommands === undefined
		this.ratelimiter = options.ratelimiter || new CommandRatelimiter()
		this.strictCommandCheck =
			options.strictCommandCheck ||
			options.strictCommandCheck === undefined
		...
		this.onInteractionCheck =
			options.onInteractionCheck || this.onInteractionCheck
		this.onInteractionCancel =
			options.onInteractionCancel || this.onInteractionCancel
		...
		if (process.env.CLUSTER_MANAGER === 'true') {
			...
			if (token instanceof ClusterClient) {
				if (process.env.CLUSTER_TOKEN !== token.token) {
					throw new Error(
						'Cluster Client must have matching tokens with the Manager!'
					)
				}
			} else {
		...
		if (
			!client ||
			!(client instanceof ClusterClient || client instanceof ShardClient)
		) {
			throw new Error(
				'Token has to be a string or an instance of a client'
			)
		}
		this.client = client
		Object.defineProperty(this.client, 'interactionCommandClient', {
			value: this
		})
		if (this.client instanceof ClusterClient) {
			for (let [shardId, shard] of this.client.shards) {
				Object.defineProperty(shard, 'interactionCommandClient', {
					value: this
				})
			}
		}
		...
		if (options.ratelimits) {
			for (let rOptions of options.ratelimits) {
				if (typeof rOptions.type === 'string') {
					...
					if (
						this.ratelimits.some(
							(ratelimit) => ratelimit.type === rType
						)
					) {
						throw new Error(
							`Ratelimit with type ${rType} already exists`
						)
					}
				}
	...
	add(
		...
		if (options instanceof InteractionCommand) {
			...
			if (options._class === undefined) {
				...
				if (!command._file) {
					Object.defineProperty(command, '_file', {
						value: options._file
					})
				}
			}
	...
	addMultiple(
		commands: Array<InteractionCommand | InteractionCommandOptions> = []
	): this {
	...
	async checkAndUploadCommands(force: boolean = false): Promise<void> {
		...
		for (let [guildId, localCommands] of this.commandsById) {
			const guildIdOrUndefined =
				guildId === LOCAL_GUILD_ID ? undefined : guildId
			if (
				force ||
				(!(await this.checkApplicationCommands(guildIdOrUndefined)) &&
					this.canUpload)
			) {
				...
				if (this.manager && this.manager.hasMultipleClusters) {
					this.manager.sendIPC(
						ClusterIPCOpCodes.FILL_INTERACTION_COMMANDS,
						{ data: commands }
					)
				}
			}
	...
	createApplicationCommandsFromRaw(
		data: Array<any>
	): BaseCollection<string, ApplicationCommand> {
		const collection = new BaseCollection<string, ApplicationCommand>()

		const shard =
			this.client instanceof ClusterClient
				? this.client.shards.first()!
				: this.client
		for (let raw of data) {
	...
	async fetchApplicationCommands(
		...
		if (!this.client.ran) {
			throw new Error(
				"Client hasn't ran yet so we don't know our application id!"
			)
		}
		let data: Array<any>
		if (this.manager && this.manager.hasMultipleClusters) {
			if (guildId) {
				data = await this.manager.sendRestRequest(
					'fetchApplicationGuildCommands',
					[this.client.applicationId, guildId]
				)
			} else {
				data = await this.manager.sendRestRequest(
			...
			if (guildId) {
				data = await this.rest.fetchApplicationGuildCommands(
					this.client.applicationId,
					guildId
				)
			} else {
				data = await this.rest.fetchApplicationCommands(
					this.client.applicationId
				)
			}
		}
	...
	async uploadApplicationCommands(
		guildId?: string
	): Promise<BaseCollection<string, ApplicationCommand>> {
		...
		const localCommands = (
			this.commandsById.get(guildId || LOCAL_GUILD_ID) || []
		).map((command: InteractionCommand) => {
			const data = command.toJSON()
			;(data as any)[DiscordKeys.ID] = command.ids.get(
		...
		const shard =
			this.client instanceof ClusterClient
				...
				: this.client
		if (guildId) {
			return shard.rest.bulkOverwriteApplicationGuildCommands(
				this.client.applicationId,
				guildId,
				localCommands
			)
		} else {
			return shard.rest.bulkOverwriteApplicationCommands(
				this.client.applicationId,
				localCommands
			)
		}
	...
	validateCommands(
		commands: BaseCollection<string, ApplicationCommand>
	): boolean {
		if (!commands.length) {
			return false
		}
	...
	async parseArgs(
		...
		if (data.isSlashCommand) {
			return this.parseArgsFromOptions(
				context,
				context.command._options,
				data.options,
				data.resolved
			)
	...
	async parseArgsFromContextMenu(
		data: InteractionDataApplicationCommand
				...
				case ApplicationCommandTypes.MESSAGE:
					{
						if (data.resolved.messages) {
							args.message = data.resolved.messages.get(
								data.targetId
							)
						}
					}
				...
				case ApplicationCommandTypes.USER:
					{
						if (data.resolved.members) {
							args.member = data.resolved.members.get(
								data.targetId
							)!
						}
    ...
    */
		...
		if (options) {
			for (let [name, option] of options) {
				...
				if (option.options) {
					const [childArgs, childErrors] =
						await this.parseArgsFromOptions(
							context,
							commandOption && commandOption._options,
							option.options,
							resolved
						)
							...
							case ApplicationCommandOptionTypes.MENTIONABLE:
								{
									if (
										resolved.roles &&
										resolved.roles.has(value)
									) {
										value = resolved.roles.get(value)
									} else if (
										resolved.members &&
										resolved.members.has(value)
									) {
										value = resolved.members.get(value)
									} else if (
										resolved.users &&
										resolved.users.has(value)
									) {
							...
							case ApplicationCommandOptionTypes.ROLE:
								{
									if (resolved.roles) {
										value =
											resolved.roles.get(value) || value
									}
								}
								break
							case ApplicationCommandOptionTypes.USER:
								{
									if (resolved.members) {
										value =
											resolved.members.get(value) || value
									} else if (resolved.users) {
										value =
											resolved.users.get(value) || value
									}
								}
								break
						}
					...
					if (commandOption) {
						if (
							commandOption.value &&
							typeof commandOption.value === 'function'
						) {
							try {
								args[label] = await Promise.resolve(
									commandOption.value(value, context)
								)
							} catch (error: any) {
								hasError = true
								errors[label] = error
					...
					hasError =
						(await this.parseDefaultArgsFromOptions(
							context,
	...
	async parseDefaultArgsFromOptions(
		...
		for (let [name, commandOption] of commandOptions) {
			...
			if (
				commandOption.default !== undefined &&
				!(label in args) &&
				!(label in errors)
			) {
	...
	async handleInteractionCreate(
		event: GatewayClientEvents.InteractionCreate
	) {
	...
	async handleApplicationCommand(
		...
		const guildIds = interaction.guildId
			? [interaction.guildId, LOCAL_GUILD_ID]
			: [LOCAL_GUILD_ID]
		for (let guildId of guildIds) {
			if (this.commandsById.has(guildId)) {
				const localCommands = this.commandsById.get(guildId)!
				if (this.strictCommandCheck) {
					command = localCommands.find(
						(cmd) => cmd.ids.get(guildId) === data.id
					)
				} else {
					command = localCommands.find(
						(cmd) =>
							cmd.name === data.name && cmd.type === data.type
					)
				}
				if (command) {
					break
		...
		if (typeof this.onInteractionCheck === 'function') {
			try {
				const shouldContinue = await Promise.resolve(
					this.onInteractionCheck(context)
				)
				if (!shouldContinue) {
					if (typeof this.onInteractionCancel === 'function') {
						return await Promise.resolve(
							this.onInteractionCancel(context)
						)
					}
					return
				}
			} catch (error: any) {
				const payload: InteractionCommandEvents.CommandError = {
					command,
					context,
					error
				}
		...
		if (typeof this.onCommandCheck === 'function') {
			try {
				const shouldContinue = await Promise.resolve(
					this.onCommandCheck(context, command)
				)
				if (!shouldContinue) {
					if (typeof this.onCommandCancel === 'function') {
						return await Promise.resolve(
							this.onCommandCancel(context, command)
						)
					}
					return
				}
		...
		if (
			this.ratelimits.length ||
			(invoker.ratelimits && invoker.ratelimits.length)
		) {
			const now = Date.now()
			{
				const ratelimits = this.ratelimiter.getExceeded(
					context,
					this.ratelimits,
					now
				)
				if (ratelimits.length) {
					...
					const payload: InteractionCommandEvents.CommandRatelimit = {
						command,
						context,
						global,
						ratelimits,
					...
					if (typeof invoker.onRatelimit === 'function') {
						try {
							await Promise.resolve(
								invoker.onRatelimit(context, ratelimits, {
									global,
									now
								})
							)
		...
		if (context.inDm) {
			...
			if (invoker.disableDm) {
				if (typeof invoker.onDmBlocked === 'function') {
					...
					const error = new Error(
						'Command with DMs disabled used in DM'
					)
					const payload: InteractionCommandEvents.CommandError = {
						command,
						context,
			...
			if (
				Array.isArray(invoker.permissionsClient) &&
				invoker.permissionsClient.length
			) {
				const failed = []
				...
				if (failed.length) {
					const payload: InteractionCommandEvents.CommandPermissionsFailClient =
						{ command, context, permissions: failed }
					this.emit(
					...
					if (typeof invoker.onPermissionsFailClient === 'function') {
						try {
							await Promise.resolve(
								invoker.onPermissionsFailClient(context, failed)
							)
						} catch (error) {
							// do something with this error?
						}
			...
			if (
				!invoker.permissionsIgnoreClientOwner ||
				!context.user.isClientOwner
			) {
				// check the user's permissions
				if (
					Array.isArray(invoker.permissions) &&
					invoker.permissions.length
				) {
					const failed = []
					...
					if (failed.length) {
						const payload: InteractionCommandEvents.CommandPermissionsFail =
							{ command, context, permissions: failed }
						this.emit(
						...
						if (typeof invoker.onPermissionsFail === 'function') {
							try {
								await Promise.resolve(
									invoker.onPermissionsFail(context, failed)
								)
							} catch (error) {
								// do something with this error?
							}
		...
		try {
			const shouldTriggerLoading =
				invoker.triggerLoadingAfter !== undefined &&
				0 <= invoker.triggerLoadingAfter
			...
			if (shouldTriggerLoading && !context.responded) {
				let data:
					| RequestTypes.CreateInteractionResponseInnerPayload
					| undefined
				...
				if (invoker.triggerLoadingAfter) {
					...
					Object.defineProperty(context, 'loadingTimeout', {
						value: timeout
					})
					timeout.start(invoker.triggerLoadingAfter, async () => {
						...
						try {
							if (
								typeof invoker.onLoadingTrigger === 'function'
							) {
								await Promise.resolve(
									invoker.onLoadingTrigger(context)
								)
							} else {
		...
		try {
			if (errors) {
				if (typeof invoker.onValueError === 'function') {
					await Promise.resolve(
						invoker.onValueError(context, args, errors)
					)
				}

				const error = new Error(
					'Command errored out while converting args'
				)
				const payload: InteractionCommandEvents.CommandError = {
					command,
					context,
			...
			if (typeof invoker.onBeforeRun === 'function') {
				const shouldRun = await Promise.resolve(
					invoker.onBeforeRun(context, args)
				)
				if (!shouldRun) {
					if (typeof invoker.onCancelRun === 'function') {
						await Promise.resolve(
							invoker.onCancelRun(context, args)
						)
					}
					return
				}
			...
			try {
				...
				const payload: InteractionCommandEvents.CommandRunError = {
					args,
					command,
					context,
					error
				...
				if (typeof invoker.onRunError === 'function') {
					await Promise.resolve(
						invoker.onRunError(context, args, error)
					)
				}
				return
			}

			const payload: InteractionCommandEvents.CommandRan = {
				args,
				command,
				context
			}
			...
			const payload: InteractionCommandEvents.CommandFail = {
				args,
				command,
				context,
				error
	...
	async handleApplicationCommandAutoComplete(
		...
		const context = new InteractionAutoCompleteContext(
			this,
			interaction,
			command,
			invoker
		...
		if (typeof invoker.onAutoComplete === 'function') {
			try {
				...
				if (typeof invoker.onAutoCompleteError === 'function') {
					await Promise.resolve(
						invoker.onAutoCompleteError(context, error)
					)
				}
				// emit the error?
			}
	...
	on(
		event: ClientEvents.COMMAND_ERROR,
		listener: (payload: InteractionCommandEvents.CommandError) => any
	): this
	on(
		event: 'commandError',
		listener: (payload: InteractionCommandEvents.CommandError) => any
	...
	on(
		event: ClientEvents.COMMAND_FAIL,
		listener: (payload: InteractionCommandEvents.CommandFail) => any
	): this
	on(
		event: 'commandFail',
		listener: (payload: InteractionCommandEvents.CommandFail) => any
	...
	on(
		event: ClientEvents.COMMAND_PERMISSIONS_FAIL_CLIENT,
		listener: (
			payload: InteractionCommandEvents.CommandPermissionsFailClient
		) => any
	): this
	on(
	...
	on(
		event: ClientEvents.COMMAND_PERMISSIONS_FAIL,
		listener: (
			payload: InteractionCommandEvents.CommandPermissionsFail
		) => any
	): this
	on(
	...
	on(
		event: ClientEvents.COMMAND_RAN,
		listener: (payload: InteractionCommandEvents.CommandRan) => any
	): this
	on(
		event: 'commandRan',
		listener: (payload: InteractionCommandEvents.CommandRan) => any
	...
	on(
		event: ClientEvents.COMMAND_RATELIMIT,
		listener: (payload: InteractionCommandEvents.CommandRatelimit) => any
	): this
	on(
	...
	on(
		event: ClientEvents.COMMAND_RUN_ERROR,
		listener: (payload: InteractionCommandEvents.CommandRunError) => any
	): this
	on(
