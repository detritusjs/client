...
export type CommandClientCommandCheck = (
	context: Context,
	command: Command
) => Promise<boolean> | boolean
export type CommandClientCommandCancel = (
	context: Context,
	command: Command
) => Promise<any> | any
export type CommandClientMessageCheck = (
	context: Context
) => Promise<boolean> | boolean
export type CommandClientMessageCancel = (
	context: Context
) => Promise<any> | any

export type CommandClientPrefixes =
	| Array<string>
	| BaseSet<string>
	| Set<string>
	| string
export type CommandClientPrefixCheck = (
	context: Context
) => CommandClientPrefixes | Promise<CommandClientPrefixes>
...
export class CommandClient extends EventSpewer {
	...
	constructor(
		...
		this.maxEditDuration = +(
			options.maxEditDuration || this.maxEditDuration
		)
		this.mentionsEnabled = !!(
			options.mentionsEnabled || options.mentionsEnabled === undefined
		)
	...
	async getAttributes(context: Context): Promise<CommandAttributes | null> {
		...
		if (!prefix) {
			const customPrefixes = await Promise.resolve(
				this.getPrefixes(context)
			)
	...
	setSubscriptions(): void {
		...
		subscriptions.push(
			this.client.subscribe(
				...
				this.handleDelete.bind(this, ClientEvents.MESSAGE_DELETE)
			)
		)
		subscriptions.push(
			this.client.subscribe(
	...
	async handle(
		...
		if (name === ClientEvents.MESSAGE_UPDATE) {
			if (this.replies.has(message.id)) {
				const { command, context, reply } = this.replies.get(
					message.id
				)!
				const payload: CommandEvents.CommandDelete = {
					command,
					context,
					reply
				}
		...
		if (typeof this.onMessageCheck === 'function') {
			try {
				const shouldContinue = await Promise.resolve(
					this.onMessageCheck(context)
				)
				if (!shouldContinue) {
					if (typeof this.onMessageCancel === 'function') {
						return await Promise.resolve(
							this.onMessageCancel(context)
						)
					}
					const error = new Error('Message check returned false')
					const payload: CommandEvents.CommandNone = {
						context,
						error
					}
		...
		try {
			...
			if (message.isEdited) {
				...
				if (this.maxEditDuration < difference) {
					throw new Error(
						'Edit timestamp is higher than max edit duration'
					)
				}
			}
		...
		if (!command.responseOptional && !message.canReply) {
			...
			const payload: CommandEvents.CommandError = {
				command,
				context,
				error
			}
		...
		if (this.ratelimits.length || command.ratelimits.length) {
			const now = Date.now()
			{
				...
				if (ratelimits.length) {
					...
					const payload: CommandEvents.CommandRatelimit = {
						command,
						context,
						global,
						ratelimits,
					...
					if (typeof command.onRatelimit === 'function') {
						try {
							await Promise.resolve(
								command.onRatelimit(context, ratelimits, {
									global,
									now
								})
							)
		...
		if (context.inDm) {
					...
					if (command.disableDmReply) {
						...
						try {
							const reply = await message.reply(
								`Cannot use \`${command.name}\` in DMs.`
							)
							this.storeReply(message.id, command, context, reply)
							const payload: CommandEvents.CommandError = {
								command,
			...
			if (
				Array.isArray(command.permissionsClient) &&
				command.permissionsClient.length
			) {
				const failed = []
				...
				if (failed.length) {
					const payload: CommandEvents.CommandPermissionsFailClient =
					...
					if (typeof command.onPermissionsFailClient === 'function') {
						try {
							const reply = await Promise.resolve(
								command.onPermissionsFailClient(context, failed)
							)
							this.storeReply(message.id, command, context, reply)
						} catch (error: any) {
							// do something with this error?
			...
			if (
				!command.permissionsIgnoreClientOwner ||
				...
				if (
					...
					if (failed.length) {
						const payload: CommandEvents.CommandPermissionsFail = {
							command,
							context,
							permissions: failed
						}
						...
						if (typeof command.onPermissionsFail === 'function') {
							try {
								const reply = await Promise.resolve(
									command.onPermissionsFail(context, failed)
								)
								this.storeReply(
									message.id,
									command,
		...
		if (typeof command.onBefore === 'function') {
			try {
				const shouldContinue = await Promise.resolve(
					command.onBefore(context)
				)
				if (!shouldContinue) {
					if (typeof command.onCancel === 'function') {
						const reply = await Promise.resolve(
							command.onCancel(context)
						)
						this.storeReply(message.id, command, context, reply)
					}
					return
		...
		const { errors, parsed: args } = await command.getArgs(
			attributes,
			context
		)
		if (Object.keys(errors).length) {
			if (typeof command.onTypeError === 'function') {
				const reply = await Promise.resolve(
					command.onTypeError(context, args, errors)
				)
				this.storeReply(message.id, command, context, reply)
			}
		...
		try {
			if (typeof command.onBeforeRun === 'function') {
				const shouldRun = await Promise.resolve(
					command.onBeforeRun(context, args)
				)
				if (!shouldRun) {
					if (typeof command.onCancelRun === 'function') {
						const reply = await Promise.resolve(
							command.onCancelRun(context, args)
						)
						this.storeReply(message.id, command, context, reply)
					}
					return
			...
			try {
				if (command.triggerTypingAfter !== -1) {
					if (command.triggerTypingAfter) {
						...
						Object.defineProperty(context, 'typingTimeout', {
							value: timeout
						})
				...
				const payload: CommandEvents.CommandRunError = {
					args,
					command,
					context,
					error,
				...
				if (typeof command.onRunError === 'function') {
					const reply = await Promise.resolve(
						command.onRunError(context, args, error)
					)
					this.storeReply(message.id, command, context, reply)
				}
				return
			...
			const payload: CommandEvents.CommandRan = {
				args,
				command,
				context,
				prefix
			...
			const payload: CommandEvents.CommandFail = {
				args,
				command,
				context,
				error,
		...
		if (this.replies.has(messageId)) {
			...
			for (let [commandId, { command, context, reply }] of this.replies) {
				if (reply.id === messageId) {
					...
					const payload: CommandEvents.CommandResponseDelete = {
						command,
						context,
						reply
					}
	...
	on(
		event: ClientEvents.COMMAND_DELETE,
		listener: (payload: CommandEvents.CommandDelete) => any
	): this
	on(
		event: 'commandDelete',
		listener: (payload: CommandEvents.CommandDelete) => any
	...
	on(
		...
		listener: (payload: CommandEvents.CommandError) => any
	): this
	on(
		event: 'commandError',
		listener: (payload: CommandEvents.CommandError) => any
	...
	on(
		...
		listener: (payload: CommandEvents.CommandFail) => any
	): this
	on(
		event: 'commandFail',
		listener: (payload: CommandEvents.CommandFail) => any
	...
	on(
		event: ClientEvents.COMMAND_NONE,
		listener: (payload: CommandEvents.CommandNone) => any
	): this
	on(
		event: 'commandNone',
		listener: (payload: CommandEvents.CommandNone) => any
	...
	on(
		...
		listener: (payload: CommandEvents.CommandPermissionsFailClient) => any
	): this
	on(
	...
	on(
		...
		listener: (payload: CommandEvents.CommandPermissionsFail) => any
	): this
	on(
	...
	on(
		...
		listener: (payload: CommandEvents.CommandRan) => any
	): this
	on(
		event: 'commandRan',
		listener: (payload: CommandEvents.CommandRan) => any
	...
	on(
		...
		listener: (payload: CommandEvents.CommandRatelimit) => any
	): this
	on(
	...
	on(
		event: ClientEvents.COMMAND_RESPONSE_DELETE,
		listener: (payload: CommandEvents.CommandResponseDelete) => any
	): this
	on(
		event: 'commandResponseDelete',
		listener: (payload: CommandEvents.CommandResponseDelete) => any
	): this
	on(
		event: ClientEvents.COMMAND_RUN_ERROR,
		listener: (payload: CommandEvents.CommandRunError) => any
	): this
	on(
