...
class ClusterClient extends Utils.EventEmitter
{
	...
	emit(client, name, event)
	{
		...
		super.emit.call(this, name, Object.assign(event, {shardId: client.gateway.shardId}));
	}
	...
	run(options)
	{
		...
		const fetchGateway = (options.url) ? Promise.resolve({url: options.url, shards: 0}) : this.rest.endpoints.fetchGatewayBot();
