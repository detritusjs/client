...
class CommandClient extends Utils.EventEmitter {
	...
	clearCommands() {
		for (let command of this.commands) {
			if (!command._file) {continue;}
			const mod = require.resolve(command._file);
			if (!mod) {continue;}
	...
	registerCommand(cmd) {
		...
		Object.defineProperty(command, '_file', {value: cmd._file || null});
		...
		return new Promise((resolve, reject) => {
			...
			for (let file of files) {
				...
				if (typeof(command) === 'function') {
					this.registerCommand({_class: command, _file: filepath});
				}
			}
		});
	}
		...
		return new Promise((resolve, reject) => {
			...
			switch (error.code) {
				case CommandConstants.Errors.RATELIMIT: {
					...
					if (typeof(payload.command.onRatelimit) === 'function') {
						return new Promise((resolve, reject) => {
							Promise.resolve(payload.command.onRatelimit(payload.context, payload.args, payload.remaining)).then(resolve).catch(reject);
