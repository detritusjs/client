...
class Member extends BaseStructure
{
	...
	addRole(roleId) {return this.client.rest.endpoints.addMemberRole(this.guildId, this.id, roleId);}

	ban(query) {return this.client.rest.endpoints.createBan(this.guildId, this.id, query);}

	edit(data) {return this.client.rest.endpoints.editMember(this.guildId, this.id, data);}
	editNick(nick) {return (this.id === this.client.user.id) ? this.client.rest.endpoints.editNick({nick}) : this.edit({nick});}

	remove() {return this.client.rest.endpoints.removeGuildMember(this.guildId, this.id);}
	removeBan() {return this.client.rest.endpoints.removeGuildBan(this.guildId, this.id);}
	removeRole(roleId) {return this.client.rest.endpoints.removeMemberRole(this.guildId, this.id, roleId);}
