...
class Webhook extends BaseStructure
{
	...
	{
		super(client, data, defaults, ['user']);
	...
	delete() {return (this.token) ? this.client.rest.endpoints.deleteWebhookToken(this.id, this.token) : this.client.rest.endpoints.deleteWebhook(this.id);}
	execute(data, compatible) {return this.client.rest.endpoints.executeWebhook(this.id, this.token, data, compatible);}
