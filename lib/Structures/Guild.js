...
class Guild extends BaseStructure
{
    constructor(client, raw)
    {
        ...
        this._channels = this._channels.map((data) => {
            var _Channel;
            switch (data.type)
            {
            ...
            this._client.channels.add(new _Channel(this._client, data));
            return data.id;
        });

        this._members = this._members.map((data) => {
            this._client.members.add(new Structures.Member(this._client, Object.assign(data, {guildId: this.id})));
            return data.id;
        });
    }
