...
class Messages extends BaseCollection
{
	...
	setExpire(expire) {return Object.defineProperty(this, 'expire', {enumerable: true, configurable: true, value: expire});}
	setLimit(limit) {return Object.defineProperty(this, 'limit', {enumerable: true, configurable: true, value: limit});}
	setType(type)
	{
		if (!Constants.Detritus.MessageCacheTypes.includes(type)) {
			throw new Error(`Invalid Cache Type, valid: ${JSON.stringify(Constants.Detritus.MessageCacheTypes)}`);
		}
		
		return Object.defineProperty(this, 'type', {enumerable: true, configurable: true, value: type});
	}

	get size() {return (this.type === 'global') ? super.size : this.reduce((size, cache) => size + cache.size, 0);}
	...
	has(id, cacheId)
	{
		...
		if (!id || this.type === 'global') {return super.has(cacheId);}
	...
	fetch(channelId, id) {return this.client.rest.endpoints.fetchMessage(channelId, id);}
