...
class Gateway
{
	constructor(client, options={})
	{
		if (!Dependencies.BaseSocket) {
			throw new Error('You must install `ws` or `uws` to be able to use the normal client!');
		}
	...
	ping(timeout)
	{
		...
		return new Promise((resolve, reject) => {
			if (!this.socket) {return reject(new Error('Socket isn\'t set up.'));}
			const data = {nonce: Utils.Snowflake.generate()};
