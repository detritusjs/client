...
class GatewayHandler
{
	constructor(client, options)
	{
		...
		options = options || {};
		this.disabledEvents = new Set((options.disabledEvents || defaults.disabledEvents).map((v) => v.toUpperCase()));
		this.memberChunksLeft = new Set();

		this.loadAllMembers = !!(options.loadAllMembers || defaults.loadAllMembers);
	}
	...
	unknown(name, data) {this.client.emit(name, data);}
	...
	{
		const payload = {user: null, member: null, old: null, new: null, guildId: data.guild_id};

		if (this.client.users.has(data.user.id)) {
			payload.user = this.client.users.get(data.user.id);
			payload.user.merge(data.user);
		} else {
			payload.user = new Structures.User(this.client, data.user);
