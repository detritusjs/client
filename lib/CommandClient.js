	...
	handle(name, event)
	{
		...
		return new Promise((resolve, reject) => {
			...
			if (command.ratelimit) {
				const ratelimitType = command.ratelimit.settings.type || 'user';
				let ratelimitId;
				...
				if (ratelimit.usages + 1 > command.ratelimit.settings.limit) {
					if (!ratelimit.replied) {
						const remaining = (ratelimit.start + (command.ratelimit.settings.duration * 1000) - Utils.Tools.now()) / 1000;
			...
			return Promise.resolve(info.command.run(message, info.args)).then((response) => {
				...
				this.emit('COMMAND_RUN_FAIL', Object.assign(info, {error, message}));
			});
		}).catch(console.error);
	}
