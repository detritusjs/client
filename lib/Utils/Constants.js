...
module.exports = {
    ...
    Endpoints: {
        ...
        REST: {
            ...
            APPLICATIONS: {
                ALL:                                           '/applications',
                ASSETS:                     (applicationId) => `/oauth2/applications/${applicationId}/assets`,
                ICON:   (applicationId, hash, format='png') => `/applications/${applicationId}/app-icons/${hash}.${format}`
            },
            ...
            CHANNELS: {
                ...
                CONVERT:                                          (channelId) => `/channels/${channelId}/convert`,
                ...
                RECIPIENTS:                                       (channelId) => `/channels/${channelId}/recipients`,
                RECIPIENT:                                (channelId, userId) => `/channels/${channelId}/recipients/${userId}`,
                SEARCH:                                           (channelId) => `/channels/${channelId}/messages/search`,
            ...
            GAMES: {
                ALL:                                   '/games',
                ASSET: (gameId, hash, format='png') => `/games/${gameId}/assets/${hash}.${format}`,
                NEWS:                      (gameId) => `/game-news?game_ids=${gameId}`
            },
            ...
            GUILDS: {
                ...
                SEARCH:                          (guildId) => `/guilds/${guildId}/messages/search`,
            ...
            USERS: {
                ...
                ACTIVITY_JOIN:         (userId, sessionId) => `/users/${userId}/sessions/${sessionId}/activity/2`,
                ACTIVITY_JOIN_INVITE:  (userId, sessionId) => `/users/${userId}/sessions/${sessionId}/activity/8/${sessionId}`, //probably wrong
                ACTIVITY_JOIN_REQUEST: (userId, sessionId) => `/users/${userId}/sessions/${sessionId}/activity/8`,
                ACTIVITY_METADATA:     (userId, sessionId) => `/users/${userId}/sessions/${sessionId}/activity/metadata`,
                ACTIVITY_SPECTATE:     (userId, sessionId) => `/users/${userId}/sessions/${sessionId}/activity/4`,
                BILLING:                                      `/users/@me/billing`,
                CHANNELS:                         (userId) => `/users/${userId}/channels`,
                CONNECTIONS:                      (userId) => `/users/${userId}/connections`,
                DELETE_ACCOUNT:                   (userId) => `/users/${userId}/delete`,
                ...
                GUILD_SETTINGS:          (userId, guildId) => `/users/${userId}/guilds/${guildId}/settings`,
                MENTIONS:                                     '/users/@me/mentions',
                MENTIONS_MESSAGE:              (messageId) => `/users/@me/mentions/${messageId}`,
                PROFILE:                          (userId) => `/users/${userId}/profile`,
                RELATIONSHIPS:                    (userId) => `/users/${userId}/relationships`
            },
