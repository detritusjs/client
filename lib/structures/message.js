			...
			switch (key) {
				...
				case 'mentions': {
					...
					for (let value of data[key]) {
						if (value.member) {
							const mdata = value.member;
							delete value.member;

							Object.assign(mdata, {user: value});

							let member;
							if (this.client.members.has(this.guildId, value.id)) {
								member = this.client.members.get(this.guildId, value.id);
								member.merge(mdata);
							} else {
								Object.assign(mdata, {guild_id: this.guildId});
								member = new Structures.Member(this.client, mdata);
							}
							this.mentions.set(value.id, member);
						} else {
							let user;
