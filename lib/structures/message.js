...
const Endpoints = Utils.Constants.Endpoints;
const MessageTypes = Utils.Constants.Discord.MessageTypes;
const SystemMessages = Utils.Constants.Discord.SystemMessages;
...
class Message extends BaseStructure {
	...
	get fromSystem() {return this.type !== MessageTypes.DEFAULT;}
	...
	get jumpLink() {
		return Endpoints.REST.URL + Endpoints.ROUTES.MESSAGE(this.guildId, this.channelId, this.id);
	}
	...
	removeMention() {return this.client.rest.removeMention(this.id);}
		...
		switch (key) {
			case 'attachments': {
				...
				for (let raw of value) {
					this.attachments.set(raw.id, new Structures.Attachment(this, raw));
				}
			}; return;
			case 'author': {
				let user;
			...
			case 'content': {
				switch (this.type) {
					case MessageTypes.RECIPIENT_ADD: {
						value = SystemMessages.RecipientAdd
					...
					case MessageTypes.RECIPIENT_REMOVE: {
						...
						if (user && user.id !== this.author.id) {
							value = SystemMessages.RecipientRemove
								.replace(/:user:/g, this.author.mention)
								.replace(/:user2:/g, user.mention);
						} else {
							value = SystemMessages.RecipientRemoveSelf.replace(/:user:/g, this.author.mention);
						}
					}; break;
					case MessageTypes.CALL: {
						value = SystemMessages.CallStarted.replace(/:user:/g, this.author.mention);
					}; break;
					case MessageTypes.CHANNEL_NAME_CHANGE: {
						value = SystemMessages.ChannelNameChange
							.replace(/:user:/g, this.author.mention)
							.replace(/:name:/g, value);
					}; break;
					case MessageTypes.CHANNEL_ICON_CHANGE: {
						value = SystemMessages.ChannelIconChange.replace(/:user:/g, this.author.mention);
					}; break;
					case MessageTypes.CHANNEL_PINNED_MESSAGE: {
						value = SystemMessages.PinnedMessage.replace(/:user:/g, this.author.mention);
					}; break;
					case MessageTypes.GUILD_MEMBER_JOIN: {
						value = SystemMessages.GuildMemberJoin[
							this.createdAtUnix % SystemMessages.GuildMemberJoin.length
						].replace(/:user:/g, this.author.mention);
					}; break;
				}
			}; break;
			case 'embeds': {
				...
				for (let i = 0; i < value.length; i++) {
					this.embeds.set(i, new Structures.Embed(this, value[i]));
				}
			}; return;
			case 'member': {
				let member;
