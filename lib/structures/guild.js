	...
	mergeValue(key, value) {
		...
		switch (key) {
			...
			case 'presences': {
				...
				for (let raw of value) {
					if (this.client.presences.has(this.id, raw.user.id)) {
						this.client.presences.get(this.id, raw.user.id).merge(raw);
					} else {
						this.client.presences.insert(this.id, new Structures.Presence(this.client, raw));
					}
				}
			}; return;
			case 'voice_states': {
