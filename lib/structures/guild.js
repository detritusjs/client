...
class Guild extends BaseStructure
{
	...
	can(permissions, member, options)
	{
		...
		const id = (member) ? member.id : this.client.user.id;
		if (!options.ignoreOwner && this.isOwner((member) ? member.id : this.client.user.id)) {return true;}

		if (member === undefined) {member = this.client.members.get(this.id, this.client.user.id);}
