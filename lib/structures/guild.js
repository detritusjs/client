...
class Guild extends BaseStructure {
	...
	get presences() {return this.client.presences.get(null, this.id);}
	...
	get jumpLink() {
		return Endpoints.REST.URL + Endpoints.ROUTES.GUILD(this.id);
	}
	...
	mergeValue(key, value) {
		...
		switch (key) {
			...
			case 'presences': {
				...
				this.client.presences.delete(null, this.id);
				for (let raw of value) {
					if (this.client.presences.has(raw.user.id, this.id)) {
						this.client.presences.get(raw.user.id, this.id).merge(raw);
					} else {
						this.client.presences.insert(new Structures.Presence(this.client, raw), this.id);
					}
				}
			}; return;
			case 'voice_states': {
