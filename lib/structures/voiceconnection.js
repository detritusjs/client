...
const stringToMimetype = Utils.MimeTypes.find;

const VoiceEncoders = Utils.Encoders;
...
class VoiceConnection extends Utils.EventEmitter {
	...
	setOpusEncoder(options) {
		...
		const opusEncoder = new VoiceEncoders.Opus(options.sampleRate, options.channels, options.application, options.mod);
	...
	fetchOpusDecoder(userId) {
		...
		const opusDecoder = new VoiceEncoders.Opus(
	...
	sendAudio(data, options) {
		...
		if (!this.gateway.udp) {throw new Error('UDP isn\'t initialized yet!');}
	...
	play(stream, options) {
		...
		return new Promise((resolve, reject) => {
			if (this.killed) {return reject(new Error('This connection has been killed'));}
			if (!this.gateway.udp) {return reject(new Error('UDP isn\'t initialized yet!'));}

			if (typeof(stream) === 'string') {
				const mimetype = stringToMimetype(stream, {filename: true});
				if (mimetype) {
